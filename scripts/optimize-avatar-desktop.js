...
// Tamaños para la imagen de avatar con mayor resolución para desktop
const avatarSizes = [
  { width: 600, suffix: 'xlarge', quality: 90 },     // Para pantallas de escritorio
  { width: 900, suffix: 'xlarge-2x', quality: 90 },  // Para pantallas de escritorio de alta densidad
  { width: 400, suffix: 'large', quality: 90 },      // Para tablets grandes
  { width: 600, suffix: 'large-2x', quality: 90 },   // Para tablets grandes de alta densidad
];

// Función para redimensionar una imagen
async function resizeImage(inputPath, outputPath, width, quality = 90) {
  try {
...
async function processAvatar() {
  ...
  for (const size of avatarSizes) {
    ...
    const result = await resizeImage(avatarPath, outputPath, size.width, size.quality);
